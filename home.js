...
function toggleMenu() {
 ...
 function validateForm() {
    const eventType = document.querySelector('[name="event-type"]').value;
    ...
    if (!eventType || !location || !checkIn || !checkOut || !guests) {
        alert("Please fill out all fields before submitting.");
        return false;  // Prevent form submission if any field is empty
    }
    
    // If all fields are filled out, allow form submission
    return true;
}
